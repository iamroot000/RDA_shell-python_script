...
function step01
{
 ...
 cd /sasprog/triggers
 if [ -f S_ARCHIVE_F_SUBS_XFILES.txt ]
 then
 rm S_ARCHIVE_F_SUBS_XFILES.txt
 echo   "S_ARCHIVE_F_SUBS_XFILES.txt deleted "
 else
 echo  "S_ARCHIVE_F_SUBS_XFILES.txt does not exist"
 fi
 if [ -f U_ARCHIVE_F_SUBS_XFILES.txt ]
 then
 rm U_ARCHIVE_F_SUBS_XFILES.txt
 echo  "U_ARCHIVE_F_SUBS_XFILES.txt deleted"
 else
 echo  "U_ARCHIVE_F_SUBS_XFILES.txt does not exist"
 fi
...
batchname=ARCHIVE_F_SUBS_XFILES
tblList=ARCHIVE_F_SUBS_LIST.txt
dtx=`date +'%y%m%d'`"   "`date +'%H%M%S'`
rrstep=""
bpsw=0
ut=U_ARCHIVE_F_SUBS_XFILES.txt
st=S_ARCHIVE_F_SUBS_XFILES.txt
if [ "$1" != "" ] && [ "$1" != "step00" ]
then
