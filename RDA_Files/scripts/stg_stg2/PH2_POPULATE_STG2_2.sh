...
function step10
{
 ...
 desc="Call SAS job"
 cd /sasprog/triggers
 if [ -f S_01_POPULATE_STG2_RECEIVABLES.txt  ]
 then
 rm S_01_POPULATE_STG2_RECEIVABLES.txt
 echo   "S_01_POPULATE_STG2_RECEIVABLES.txt deleted "
 else
 echo  "S_01_POPULATE_STG2_RECEIVABLES.txt does not exist"
 fi
 if [ -f U_01_POPULATE_STG2_RECEIVABLES.txt ]
 then
 rm U_01_POPULATE_STG2_RECEIVABLES.txt
 echo  "U_01_POPULATE_STG2_RECEIVABLES.txt deleted"
 else
 echo   "U_01_POPULATE_STG2_RECEIVABLES.txt does not exist"
 fi
 $swcode $swuser -c "${dirlog} $exesas /sasprog/${lev}/stg_stg2/01_POPULATE_STG2_RECEIVABLES.sas"
 rcode=$?
 rrstep=""
 logrc
 }
...
function step13
{
 ...
 if [ -f $st ]
 then
 touch S_PH2_01_POPULATE_STG2_FINANCIAL_INST_CREDIT_ASSESS.txt S_PH2_POPULATE_STG2_2.txt
 else
...
batchname=PH2_POPULATE_STG2_2
dtx=`date +'%y%m%d'`"   "`date +'%H%M%S'`
rrstep=""
bpsw=0
ut=U_01_POPULATE_STG2_FINANCIAL_INST_CREDIT_ASSESS.txt
st=S_01_POPULATE_STG2_FINANCIAL_INST_CREDIT_ASSESS.txt
if [ "$1" != "" ] && [ "$1" != "step00" ]
then
